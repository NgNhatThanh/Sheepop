...
import com.app.bdc_backend.model.dto.response.OrderPageResponse;
...
public class OrderController {
    ...
    public ResponseEntity<?> placeOrder(@RequestBody Map<String, Object> body) {
        ...
        order.setAddress(to);
        order.setUser(user);
        List<Map<String, Object>> shops = (List<Map<String, Object>>) body.get("shop_orders");
        long totalPay = 0;
    ...
    public ResponseEntity<?> getOrderList(@RequestParam(value = "type") int type,
        ...
        User user = userService.findByUsername(username);
        return switch (type) {
            case 0 -> ResponseEntity.ok(getAllOrders(user, offset, limit));
            case 1 -> ResponseEntity.ok(getPaymentPendingOrders(user, offset, limit));
            case 2 -> ResponseEntity.ok(getOrderByShopOrderStatus(user,
                    List.of(ShopOrderStatus.PENDING),
                    offset,
                    limit));
            case 3 -> ResponseEntity.ok(getOrderByShopOrderStatus(user,
                    List.of(ShopOrderStatus.PREPARING),
                    offset,
                    limit));
            case 4 -> ResponseEntity.ok(getOrderByShopOrderStatus(user,
                    List.of(ShopOrderStatus.SENT, ShopOrderStatus.DELIVERED),
                    offset,
                    limit));
            case 5 -> ResponseEntity.ok(getOrderByShopOrderStatus(user,
                    List.of(ShopOrderStatus.COMPLETED, ShopOrderStatus.RATED),
                    offset,
                    limit));
            case 6 -> ResponseEntity.ok(getOrderByShopOrderStatus(user,
