...
export default function ShopOrder(){
    ...
    const fetchShopOrders = (type, reset = false) => {
        ...
        setOrders([])
        setIsLoading(true)
        fetchWithAuth(`${BASE_API_URL}/v1/admin/order/get_list?type=${type}&page=${page - 1}&limit=${limit}&filterType=${filterType}&sortType=${sortType}&keyword=${searchQuery}`, 
            window.location, 
            true)
            .then(res => res.json())
            .then(res => {
        ...
        <div>
            ...
            <div className="w-full bg-white shadow-md sticky top-12 z-1 flex justify-center rounded-sm">
                ...
                <button
                    ...
                    onClick={() => navigate(`/admin/order?type=${index}`)}
                >
                    {tab}
                </button>
                ))}
                ...
                <table className="table-fixed w-full border-collapse border border-gray-300">
                    <thead>
                        <tr className=" w-full">
                            ...
                            <th className="border border-gray-300 p-2 w-1/2">Sản phẩm</th>
                        ...
                        {orders.map((shopOrder) => (
                            <tr key={shopOrder.id} className="border-b border-gray-300">
                                ...
                                <td className="items-center border-r border-gray-300">
                                    <div className="p-5">
                                        <p className="mb-2 text-sm font-semibold text-gray-600 hover:text-gray-800">
                                            Người mua: {shopOrder.buyerName}
                                        </p>

                                        <div className="flex gap-1 w-fit cursor-pointer items-center">
                                            <p className="text-sm text-gray-600 hover:text-gray-800">Cửa hàng: {shopOrder.name}</p>
                                            <ImNewTab/>
                                        </div>
                                        ...
                                        {shopOrder.items.map((item) => (
                                            <div key={item.id} className="flex mt-2">
                                                ...
                                                />
                                                <div className="ml-4 flex-1 mr-5">
