...
export default function ShopPage(){
    ...
    const fetchShop = () => {
        fetchWithAuth(`${BASE_API_URL}/v1/shopinfo/get_info?username=${username}`, null, false)
            .then(res => res.json())
            .then(res => {
                if(res.message){
    ...
    const fetchProducts = () => {
        ...
        setIsLoading(true)
        setIsEmpty(false)
        fetch(`${BASE_API_URL}/v1/shopinfo/get_product_list?shopId=${shopInfo.id}${selectedCategory ? '&categoryId=' + selectedCategory : ''}&sortBy=${sort.sortBy}&order=${sort.order}&page=${page}&limit=${limit}`)
            .then(res => res.json())
            .then(res => {
    ...
    useEffect(() => {
        fetchProducts()
    }, [shopInfo, sort, selectedCategory, page])
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            {shopInfo && <CategoryNav selectedCat={selectedCategory} setCategory={setSelectedCategory} categories={shopInfo.shopCategories} />} 
                ...
                {!isEmpty ? 
                    ...
                    : (
                        <div className="w-fullh-100 mt-3 flex items-center justify-center">
                            ...
                            <VscInbox className="text-blue-500 text-8xl mx-auto mb-2"/>
